...
```
영상 업로드  →  (품질 검증)  →  AI 분석(비동기)  →  결과 리포트 제공
```
...
현미경 영상 입력
      │
  ① 객체 탐지     YOLO11 모델            → 프레임별 정자 탐지
      │
  ② 객체 추적     ByteTrack 알고리즘     → 동일 정자 ID·궤적 복원
      │
  ③ 키네마틱 계산  CASA 지표 산출         → 운동학 지표(VCL/VSL/ALH 등)
      │
  ④ 운동성 분석   Ridge + RandomForest   → 전진·비전진·비운동 비율 예측
      │
...
(데모 실행 방법은 아래 [Quick Start](#-quick-start) 참고)
